...
namespace JDGuardian.Helpers
{
    static class ThreadHelper
    {
        ...
        public static void Run<T>(IEnumerable<T> source, Action<T> action)=> Run(source, Environment.ProcessorCount, action);
        ...
        /// <param name="action">处理集合元素的委托</param>
        /// <returns>异步任务</returns>
        public static void Run<T>(IEnumerable<T> source, int threadLimit, Action<T> action)
        {
            ConcurrentQueue<T> datas = new ConcurrentQueue<T>(source);
            var tasks = Enumerable.Range(0, threadLimit).Select(t => Task.Run(() =>
            {
